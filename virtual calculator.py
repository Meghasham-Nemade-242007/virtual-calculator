...
equation = ""
delay_counter = 0
pressed_button = None  # Stores the last pressed buttonq
hovered_button = None  # Stores the currently hovered button
...
while True:
    ...
    if hands:
        ...
        if length > 50:
            # Check if the index finger is on a button
            button = check_button_click(index_finger, buttons)

            # If the finger is resting on a button and it's a new button (not the previously hovered button)
            if button and button != hovered_button and delay_counter == 0:
                pressed_button = button  # Store pressed button for visual feedback

                if button == "=":
                    try:
                        equation = str(eval(equation.replace("^", "**")))  # Evaluate the equation with '^' as '**'
                    except:
                        equation = "Error"
                elif button == "C":
                ...
                # Update hovered_button to the current button
                hovered_button = button
                delay_counter = 1  # Add delay to prevent multiple presses

        else:
            # If the finger is not on a button, reset hovered_button
            hovered_button = None
